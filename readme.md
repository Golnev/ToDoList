...
ToDoList имеет возможность добавления нескольких задач, показывать список задач, показывать определенную задачу, редактировать определенную задачу, удалять задачу и записывать список задач в txt файл.
...
Пароль не отоброжается при вводе.
...
Путь у config.ini: \docs\config.ini
...
После ввода пароля отборажается меню ToDoList.
Меню показывает все возможные операции и просит выбрать одну по числу.
...
Отображает список Ваших задач из базы данных и из json-сервера.
...
Если задая в списке нет, пишет, что список задач пуст.
...
Добовляет задачу(задачи) в ToDoList.
Вызывается вводом 2.

Необходимо ввести количество задач, которое хотите ввести, день неделя (сокращенно), приритет (от 0 до 100), название задачи (больше 7 символов).
...
После выводится сообщение, что задача добавлено успешно в базу данных и в json-сервер.
...
Если задачи с таким номер нет, выводит сообщение, что данная задача не найдена.
...
Редактирует задачу по введденному номеру.
Вызывается вводом 4.
...
При записи необходимо указать путь, где необходмо создать файл, с именем файла с расширением .txt.
